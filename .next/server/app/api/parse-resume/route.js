"use strict";(()=>{var e={};e.id=310,e.ids=[310],e.modules={399:e=>{e.exports=require("next/dist/compiled/next-server/app-page.runtime.prod.js")},517:e=>{e.exports=require("next/dist/compiled/next-server/app-route.runtime.prod.js")},4193:e=>{e.exports=require("pdf-parse")},3547:e=>{e.exports=import("@react-pdf/renderer")},4300:e=>{e.exports=require("buffer")},2361:e=>{e.exports=require("events")},7147:e=>{e.exports=require("fs")},7718:e=>{e.exports=require("node:child_process")},6005:e=>{e.exports=require("node:crypto")},7561:e=>{e.exports=require("node:fs")},3977:e=>{e.exports=require("node:fs/promises")},9411:e=>{e.exports=require("node:path")},1747:e=>{e.exports=require("node:readline")},4492:e=>{e.exports=require("node:stream")},6402:e=>{e.exports=require("node:stream/promises")},7261:e=>{e.exports=require("node:util")},2037:e=>{e.exports=require("os")},1017:e=>{e.exports=require("path")},2781:e=>{e.exports=require("stream")},7310:e=>{e.exports=require("url")},3837:e=>{e.exports=require("util")},7606:(e,t,r)=>{r.r(t),r.d(t,{originalPathname:()=>h,patchFetch:()=>f,requestAsyncStorage:()=>c,routeModule:()=>u,serverHooks:()=>m,staticGenerationAsyncStorage:()=>d});var a={};r.r(a),r.d(a,{POST:()=>p});var n=r(9303),s=r(8716),o=r(670),i=r(7070);let l=new(r(3037)).ZP;async function p(e){let t,a,n,s;try{let r=(await e.formData()).get("file");if(!r||"string"==typeof r)throw Error("no file");t=r}catch{return i.NextResponse.json({error:"invalid_request",message:"No file was received."},{status:400})}if(!t.name||!t.name.includes("."))return i.NextResponse.json({error:"invalid_request",message:"The uploaded file must have a .pdf or .docx extension."},{status:400});let o=t.name.split(".").pop()?.toLowerCase(),p="pdf"===o||"application/pdf"===t.type,u="docx"===o||"application/vnd.openxmlformats-officedocument.wordprocessingml.document"===t.type;if(!p&&!u)return i.NextResponse.json({error:"unsupported_type",message:"Only .pdf and .docx files are supported."},{status:422});let c=p?"pdf":"docx",d=1;try{let e=Buffer.from(await t.arrayBuffer());if(p){let{PDFParse:t}=await Promise.resolve().then(r.t.bind(r,4193,23)),n=new t({data:e}),s=await n.getText();if(a=s.text??"",d=s.numpages??1,a.trim().length<30)return i.NextResponse.json({error:"scanned_pdf",message:"This PDF appears to be a scanned image rather than a text-based PDF, so we can't extract the text. Try uploading a .docx version or a text-based PDF instead."},{status:422})}else{let t=await r.e(699).then(r.t.bind(r,7699,19)),o=(await t.convertToHtml({buffer:e})).value??"";a=o.replace(/<li[^>]*>/gi,"\n- ").replace(/<\/li>/gi,"").replace(/<\/p>|<br\s*\/?>/gi,"\n").replace(/<[^>]+>/g,"").replace(/&amp;/g,"&").replace(/&lt;/g,"<").replace(/&gt;/g,">").replace(/&nbsp;/g," ").replace(/&#(\d+);/g,(e,t)=>String.fromCharCode(Number(t))).replace(/\n{3,}/g,"\n\n").trim(),s=function(e){let t;let r=[],a=/<h([1-6])[^>]*>([\s\S]*?)<\/h\1>/gi;for(;null!==(t=a.exec(e));){let a=e.slice(0,t.index).match(/<(h[1-6]|p|li)[>\s]/gi)??[],n=t[2].replace(/<[^>]+>/g,"").trim();n&&r.push({sectionHeading:n,paragraphIndex:a.length})}return r}(o);let l=o.search(/<h[2-6][\s>]/i),p=l>0?o.slice(0,l):o.slice(0,3e3);if(n=(function(e){let t;let r=[],a=/<a\s[^>]*href="([^"]+)"[^>]*>([\s\S]*?)<\/a>/gi;for(;null!==(t=a.exec(e));){let e=t[1].trim(),a=t[2].replace(/<[^>]+>/g,"").replace(/&amp;/g,"&").replace(/&lt;/g,"<").replace(/&gt;/g,">").replace(/&nbsp;/g," ").replace(/&#(\d+);/g,(e,t)=>String.fromCharCode(Number(t))).trim();e&&a&&r.push({label:a,url:e})}return r})(p).filter(e=>!e.url.toLowerCase().startsWith("mailto:")),a.trim().length<30)return i.NextResponse.json({error:"empty_file",message:"This file appears to be empty or couldn't be read. Please try a different file."},{status:422})}}catch{return i.NextResponse.json({error:"parse_failed",message:"This file couldn't be read. It may be corrupted or in an unexpected format. Please try a different file."},{status:422})}try{let e=await l.messages.create({model:"claude-sonnet-4-6",max_tokens:4096,system:function(e){let t=e&&e.length>0?`- "links": an array of { "label": string, "url": string | null } objects for contact/social links only (LinkedIn, Portfolio, GitHub, personal website, etc.). The following hyperlinks were extracted directly from the document source and their URLs are exact — use them verbatim for this field only:
${e.map(e=>`  { "label": "${e.label}", "url": "${e.url}" }`).join("\n")}
  Match each contact/social link label (e.g. "LinkedIn", "Portfolio", "GitHub") to the closest entry above and use that url value. Only set "url" to null for a contact link type that does not appear in this list at all. Do NOT use this list to modify or append URL text to any other section's content — all section content must be taken from the raw extracted text verbatim.`:`- "links": an array of { "label": string, "url": string | null } objects, one per link found (LinkedIn, portfolio, GitHub, personal website, etc.). Use the actual URL from the source text. The source text may contain markdown-style links in the form [Label](https://...) — when you see this pattern, use the text inside the square brackets as the label and the URL inside the parentheses as the url value. Only set "url" to null when no URL genuinely exists anywhere in the source text for that link — never write an explanatory phrase in place of a URL, and never guess or invent a URL that is not explicitly present in the source text.`;return`You are a resume parser. You receive raw text extracted from a resume file.
...
}`}(n),messages:[{role:"user",content:a.slice(0,1e5)}]}),t=("text"===e.content[0].type?e.content[0].text.trim():"").replace(/^```(?:json)?\n?/,"").replace(/\n?```$/,"").trim(),o=JSON.parse(t);if("docx"===c&&Array.isArray(o?.sections))try{let e=(await Promise.resolve().then(r.t.bind(r,1159,23))).default,{renderToBuffer:t}=await Promise.resolve().then(r.bind(r,3547)),{ATSTemplate:a}=await r.e(396).then(r.bind(r,9396)),n=e.createElement(a,{sections:o.sections}),s=new Promise((e,t)=>setTimeout(()=>t(Error("PDF page-count render timed out")),3e4)),i=await Promise.race([t(n),s]),{PDFParse:l}=await Promise.resolve().then(r.t.bind(r,4193,23));d=(await l(i)).numpages??d}catch(e){console.warn("[parse-resume] DOCX real page count render failed, keeping estimate of 1:",e)}return i.NextResponse.json({structured:o,fileType:c,pageCount:d,paragraphPositions:s})}catch(e){return console.error("[parse-resume] outer catch:",e),i.NextResponse.json({error:"claude_error",message:"Something went wrong while analyzing your resume. Please try again."},{status:500})}}let u=new n.AppRouteRouteModule({definition:{kind:s.x.APP_ROUTE,page:"/api/parse-resume/route",pathname:"/api/parse-resume",filename:"route",bundlePath:"app/api/parse-resume/route"},resolvedPagePath:"/Users/juhisharma/Documents/GitHub/JDResumeCustomizer/src/app/api/parse-resume/route.ts",nextConfigOutput:"",userland:a}),{requestAsyncStorage:c,staticGenerationAsyncStorage:d,serverHooks:m}=u,h="/api/parse-resume/route";function f(){return(0,o.patchFetch)({serverHooks:m,staticGenerationAsyncStorage:d})}}};var t=require("../../../webpack-runtime.js");t.C(e);var r=e=>t(t.s=e),a=t.X(0,[948,91],()=>r(7606));module.exports=a})();